			...
			for(String s : section.getKeys(false)) {
				...
				if(section.contains(s + ".generationSound")) {
					...
					if(soundString != null && !soundString.equalsIgnoreCase("none")) {
						...
						try {
							sound = org.bukkit.Sound.valueOf(soundString.toUpperCase());
						} catch (IllegalArgumentException e) {
							org.bukkit.NamespacedKey key = org.bukkit.NamespacedKey.fromString(soundString.toLowerCase().replace("minecraft:", ""));
							sound = key != null ? org.bukkit.Registry.SOUNDS.get(key) : null;
						}
						if(sound != null) {
				...
				if(section.contains(s + ".particleEffect")) {
					...
					if(particle != null && !particle.equalsIgnoreCase("none")) {
						try {
							org.bukkit.Particle bukkitParticle = org.bukkit.Particle.valueOf(particle.toUpperCase());
							mode.setParticleEffect(bukkitParticle);
						} catch (IllegalArgumentException e) {
							plugin.error("The particle effect " + particle + " does not exist", true);
						}
					}
				}
